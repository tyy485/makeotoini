...
> 把音频文件夹拖进来，一键生成 oto.ini，支持自动转码 + 静音检测 + 别名批量处理 + 多语言音源 + character.txt 生成 + 呼吸声智能识别
...
- 🌬️ **呼吸声智能识别** — 自动识别 `br`/`呼`/`吸`/`breathe`/`breath` 或 `b+数字` 格式的呼吸声文件，自动分配别名（支持自定义模板，如 `breath_x` 自动编号）
...
4. 生成的 `oto.ini` 和 `character.txt`（如启用）会在同一目录下
...
[2/52] 处理: b1.wav
   🌬️  呼吸声文件: b1.wav -> 别名: breath_1
✅ 已处理: b1.wav (别名: breath_1, 时长: 300ms, 静音: 0ms, offset: 0ms)

[3/52] 处理: b2.wav
   🌬️  呼吸声文件: b2.wav -> 别名: breath_2
✅ 已处理: b2.wav (别名: breath_2, 时长: 280ms, 静音: 0ms, offset: 0ms)
